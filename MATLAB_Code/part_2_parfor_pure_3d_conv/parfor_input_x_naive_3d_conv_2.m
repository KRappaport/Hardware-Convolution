function resultant = parfor_input_x_naive_3d_conv_2(input,kernel)
...
%we need to padd each edge by the lower half of kernals dimension size
...
%sme size as the original input

resultant = zeros(in_x,in_y,in_z);


%now for the fun....
%A:move the kernal on input via 3 loops
...
%ie "z" = outtermost loop, then "y" then "x" = innermost

%it goes only dim_input times in each direction

%B: start with x then y then z direction movemnt
%x: left to right (neg to pos)
%y: up to down (pos to neg)
%z: front to back (pos to neg)

% this is dependant on the size of the kernal dimensions

%flip the kernal along the "x"  and "y" axis to match
...
for z = 1 : in_z
            ...
            for m_z = 1 : ker_z
                ...
                for m_y = 1 : ker_y
                    ...
                    for m_x = 1 : ker_x
                        ...
                        %of the kernal along the padded_input (x)
    ...
    end %end of parfor y_in
end %end of parfor in_z
