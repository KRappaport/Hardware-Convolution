...
int main() {
    ...
    FRESULT res;
    res = f_chdir("test_data/var_ker/depth3");
    if (res != FR_OK) {
    ...
    for (run_amount = 1; run_amount <= 10; run_amount++) {
        sprintf(kernel_file, "ker%d_32x32x3_%d.tdatb", KER_SZ, run_amount);
        int kernel_size;
        float *kernel;
        ...
        sprintf(image_file, "img_32x32x3_%d.tdatb", run_amount);
        int image_size;
        float *image;
        ...
        sprintf(result_file, "MATLAB/res_32x32x3_ker%d_%d.tdatb", KER_SZ, run_amount);
        float *cmpr_result;
    ...
    printf("Execution time for kernel size %d: %.9f\n", KER_SZ, execution_times);
    printf("Finished convolutions!\n");
    FIL fp;
    char time_file[40];
    sprintf(time_file, "0:/var_ker%d_tm%d.tmb", KER_SZ, FREQHZ);
